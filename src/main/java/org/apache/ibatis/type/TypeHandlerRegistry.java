...
public final class TypeHandlerRegistry {
    ...
    //根据指定的Java类型和JdbcType类型查找相应的TypeHandler对象
    ...
    private <T> TypeHandler<T> getTypeHandler(Type type, JdbcType jdbcType) {
        ...
        if (jdbcHandlerMap != null) {
            ...
            if (handler == null) {
                //如采jdbcHandlerMap只注册了一个TypeHandler，则使用此TypeHandler对象
    ...
    public <T> void register(TypeHandler<T> typeHandler) {
        ...
        // 获得 @MappedTypes 注解，并根据自MappedTypes注解指定的Java类型进行注册
        ...
        // 这需妥我们的TypeHandler实现类同时继承TypeReference这个抽象类
    ...
    private void register(Type javaType, JdbcType jdbcType, TypeHandler<?> handler) {
        ...
        if (javaType != null) {//检测是否明确指定了TypeHandler能够处理的Java类型
            ...
            // 将TypeHandler对象注册到TYPEHANDLER MAP集合
            map.put(jdbcType, handler);
        }
        // 向ALL_TYPEHANDLERS_MAP集合注册TypeHandler类型和对应的TypeHandler对象
    ...
    public void register(Class<?> typeHandlerClass) {
        ...
        if (mappedTypes != null) {
            ...
            for (Class<?> javaTypeClass : mappedTypes.value()) {
                //／经过强制类型转换以及使用反射创建TypeHandler对象之后，交由重载的register继续处理
