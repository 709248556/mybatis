...
public class MapperMethod {
    ...
    //最核心的方法是execute（）方法，它会根据SQL语句的类型调用SqI Session对应的方法完成数据库操作。
    public Object execute(SqlSession sqlSession, Object[] args) {
        Object result;
        switch (command.getType()) {//调用SqlSession对应的方法
            case INSERT: {
                // 转换参数
                Object param = method.convertArgsToSqlCommandParam(args);//将用户传入的实参与指定参数名称关联起来
                // 执行 INSERT 操作
                // 转换 rowCount
    ...
    private void executeWithResultHandler(SqlSession sqlSession, Object[] args) {
        ...
        if (!StatementType.CALLABLE.equals(ms.getStatementType()) // 校验存储过程的情况。不符合，抛出 BindingException 异常
                && void.class.equals(ms.getResultMaps().get(0).getType())) {//当使用ResultHandler处理结采集时，必须指定ResultMap或ResultTyp
    ...
    public static class MethodSignature {
        ...
        public MethodSignature(Configuration configuration, Class<?> mapperInterface, Method method) {
            // 初始化 returnType 属性
            Type resolvedReturnType = TypeParameterResolver.resolveReturnType(method, mapperInterface);//解析方法的返回值类型
            ...
            // 初始化 mapKey
            this.mapKey = getMapKey(method);//若MethodSignature对应方法的返回值是Map且指定了＠MapKey注解，则使用getMapKey（）方法处理
            // 初始化 returnsMap
        ...
        //负责将args［］数纽（用户传入的实参列表）转换成SQL语句对应的参数列表
